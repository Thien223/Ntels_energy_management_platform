...
def building_energy_contract_optimization(building_id, contract_start_time='2019-03-04 00:00:00',
    ...
    while True:
        try:
            ...
            history_data_consumption, _ = load_energy_data_from_HTTPS_API(temp_config, building_ids=[building_id])
            ### resample to fill missing data
            building_data = history_data[0].resample('15T').sum()
            building_data_consumption= history_data_consumption[0].resample('15T').sum()
    ...
    for idx in assignment[cluster_idx]:
        similar_patterns.append(building_data_consumption[str(days[idx])]['Power'])
    ...
    predicted_building_energy = copy.deepcopy(building_data_consumption)
    ## alter data by adding gaussian residual
    for old_date in old_dates:
        try:
            predicted_building_energy.loc[old_date]['Power'] = np.array(building_data_consumption[old_date]['Power']) + np.array(
                residual_gauss)
    ...
    temp_df = copy.deepcopy(building_data_consumption)
