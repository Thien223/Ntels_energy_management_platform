...
def load_energy_data_from_HTTPS_API(config, period='15m', building_ids=None):
...
def load_peak_data_from_HTTPS_API(config_, period='15m', building_ids=None):
    ### get energy data using web API
    # 'https://210.219.151.169:18080/NISBCP/urbanmap/peak/getPeakTrend.ajax?startDate=20180304000000&endDate=20190304234500&period=15m&bid=B0050'
    login_sess = login(config_)
    host = config_.ntels_host
    url = host + '/NISBCP/urbanmap/peak/getPeakTrend.ajax?'
    pad = "&"
    ...
    building_list = []
    if building_ids is None:  ### get data of all building
        building_ids = get_building_ids(config_)
        # db = connect_to_data_base(config)
        # cursor = db.cursor()
        # query = 'SELECT bld_id FROM nisbcp.t_buildings;'
        # cursor.execute(query)
    ...
    for idx, building_id in enumerate(building_ids):
        ...
        if empty == False:
            for log in json:
                dates.append(log["logdate"])
                energy_values.append(log["peak"])
            dates = pd.to_datetime(dates)
            data = pd.DataFrame(energy_values, index=dates, columns=["Power"])
            data_list.append(data)
            building_list.append(building_id)
        else:
            print('Building {} does not have peak energy data from {} to {}'.format(building_id, start_time, end_time))
    return data_list, building_list
...
def sequentialize_dataframe(dataframe, window_size=12):
    # data_ = add_is_holiday_column(dataframe)
    import copy
    temp_df = copy.deepcopy(dataframe)
    for idx in range(1, window_size + 1):
        temp_df['Power_before_{}'.format(idx)] = temp_df['Power'].shift(idx)
    data_ = add_weekday_column(temp_df)
    input_sequence = data_.dropna().drop('Power', axis=1)
    output_sequence = data_.dropna()[['Power']]

    input_sequence_np = input_sequence.values
    output_sequence_np = output_sequence.values
    return input_sequence_np, output_sequence_np
...
def get_today_historical_temperature(date):
    date='2019-11-13'
    ...
    his_url = 'http://www.weather.go.kr/weather/observation/currentweather.jsp?tm={}&type=t11&mode=0&auto_man=m&stn=119'.format(date)
    his_source_code = _get_html_source_code(his_url)
    his_soup = BeautifulSoup(his_source_code, 'html.parser')
    his_data_table = his_soup.find_all('table',class_='table_develop3')[0]
    sub_table = his_data_table.find_all('tbody')[0]
    data_row = sub_table.find_all('tr')[0]
...
def update_temperature_from_kma_API_to_csv():
    ...
    dates = pd.date_range(today, end_time, freq='D')
    dates = sorted(set(dates.date))
    ...
    today_temp = get_today_historical_temperature(today)

    new_temp = new_temp.combine_first(pd.DataFrame(today_temp['Temperature']))
    ### exporting
    try:
